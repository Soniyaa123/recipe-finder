...
label $w.msg -font $font -wraplength 5i -justify left -text "This window shows three ways of using bitmaps or photos in radiobuttons and checkbuttons.  On the left are two radiobuttons, each of which displays a bitmap and an indicator.  In the middle is a checkbutton that displays a different image depending on whether it is selected or not.  On the right is a checkbutton that displays a single bitmap but changes its background color to indicate whether or not it is selected."
pack $w.msg -side top
...
image create bitmap flagup \
	-file [file join $tk_demoDirectory photos flagup.xbm] \
	-maskfile [file join $tk_demoDirectory photos flagup.xbm]
image create bitmap flagdown \
	-file [file join $tk_demoDirectory photos flagdown.xbm] \
	-maskfile [file join $tk_demoDirectory photos flagdown.xbm]
...
checkbutton $w.frame.b2 \
	-bitmap @[file join $tk_demoDirectory photos letters.xbm] \
...
radiobutton $w.frame.left.b4 \
	-bitmap @[file join $tk_demoDirectory photos noletter.xbm] \
