...
proc tixDirTree:ConstructWidget {w} {
    ...
    # We must creat an extra copy of these photos to avoid flashes on
    # the screen when user changes directory
    #
    set data(photos) [image create compound -window $data(w:hlist)]
    $data(photos) add image -image [tix getimage act_fold]
    $data(photos) add image -image [tix getimage folder]
    $data(photos) add image -image [tix getimage openfold]
}
